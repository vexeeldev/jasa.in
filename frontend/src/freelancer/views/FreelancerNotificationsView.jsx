...
import { Bell, CheckCircle, AlertCircle, Package, DollarSign, Star, X, Loader2, MessageCircle, Truck, Users } from 'lucide-react';
...
const FreelancerNotificationsView = () => {
  ...
  const getIcon = (type) => {
    switch (type) {
      case 'order': return <Users className="w-5 h-5 text-emerald-500" />;
      case 'cancellation': return <AlertCircle className="w-5 h-5 text-red-500" />;
      case 'completed': return <DollarSign className="w-5 h-5 text-green-500" />;
      case 'delivery': return <Package className="w-5 h-5 text-blue-500" />;
      case 'revision': return <MessageCircle className="w-5 h-5 text-orange-500" />;
  ...
  const handleNotificationClick = (notif) => {
    ...
    if (notif.TYPE === 'order' || notif.TYPE === 'delivery' || notif.TYPE === 'revision' || notif.TYPE === 'completed') {
      ...
      if (orderMatch) {
        navigate(`/freelancer/orders/${orderMatch[1]}`);
      }
    ...
    <div className="max-w-3xl mx-auto px-4 py-10">
      ...
      {notifications.length === 0 ? (
        <Card className="text-center py-16">
          ...
          <p className="text-gray-500">Akan ada notifikasi saat ada pesanan baru</p>
        </Card>
      ) : (
...
export default FreelancerNotificationsView;